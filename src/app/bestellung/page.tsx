    ...
    try {
      const orderPayload = {
        ...
        items: cart.map((item) => ({
          ...
          price: item.priceFormatted || (typeof item.price === "number" ? item.price.toFixed(2).replace(".", ",") : String(item.price)),
          lineTotal: ((typeof item.price === "number" ? item.price : parseFloat(String(item.price).replace(",", "."))) * item.quantity).toFixed(2).replace(".", ","),
        })),
